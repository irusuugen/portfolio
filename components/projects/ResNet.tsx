...
export default function ResNet({ onClose }: { onClose: () => void }) {
  return (
    <div className="relative w-full h-full bg-background text-primary flex flex-col overflow-y-auto">
      ...
      <div className="text-justify w-full max-w-7xl mx-auto px-8 md:px-16 py-20">
        <div className="flex flex-row gap-8 items-center">
          ...
          <a
            href="https://github.com/DaGarlic/ResNet"
            target="_blank"
        ...
        <p className="text-base opacity-80">
          <i>Top 10 Finalists of NTU Deep Learning Week 2026</i>
        </p>
        <div className="flex flex-wrap gap-2 mt-4 mb-12">
          {["React", "Python", "PyTorch", "OpenAI API"].map((tech) => (
            <span
              key={tech}
        ...
        <p className="mb-12">
          ResNet is a crisis response system that uses social media posts to
          obtain clusters of affected areas, the level of damage done to the
          clusters, and the type of need that the clusters need most, in order
          to help crisis responders plan out their route. We utilised ML models
          such as DBSCAN, CLIP Vision Transformer, OpenAI’s API, and our own
          multi-layer perceptron to collate key analytics and insights into the
          affected region.
        </p>

        <h2 className="md:text-5xl text-4xl mb-2">Demo Video</h2>
        <video width="620" height="240" controls preload="metadata">
          <source src="/resnetdemo.mp4" type="video/mp4" />
        </video>
      </div>
    </div>
  );
